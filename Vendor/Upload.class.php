...
class Upload {
     ...
     * 上传单个文件并覆盖七牛文件
     * @param $file
     * @param $onlineFileName 如果要覆盖七牛的文件，这个必须字段
     * @return array|bool
     */
    public function uploadCoverOne($file,$onlineFileName){
        if(empty($onlineFileName)) return false;
        $info = $this->upload(array($file),$onlineFileName);
        return $info ? $info[0] : $info;
    }
     ...
     * @onlineFileName 如果要覆盖七牛的文件，这个必须字段
     */
    public function upload($files='',$onlineFileName = "") {
        if('' === $files){
            $files  =   $_FILES;
        }
        if(empty($files)){
        ...
        foreach ($files as $key => $file) {
            ...
            if(!empty($onlineFileName)){
                $num = strpos($onlineFileName,'_');
                $file['savename']  = substr($onlineFileName,$num+1);
                $file['savepath']  = substr($onlineFileName,0,$num)."/";
            }
